<template>
	...
	<view class="container u-skeleton" :style="{'--theme-color':themeColor,'--theme-font-color':themeFontColor}">
		<u-navbar :z-index="'999999'" title-width="150" title="授课详情" >
			<template #right>
		...
		<view v-if="isVideo">
			...
			<video class="video" :src="videoUrl"
				@error="videoErrorCallback" :danmu-list="danmuList" enable-danmu :danmu-btn="false" controls direction @play="playStatus" @pause="pauseStatus">
			</video>
			<!-- #endif -->
			<!-- #ifdef H5 -->
			<!-- <view id="myVideo" class=""></view> -->
			<!-- #endif -->
		</view>
		<view v-if="isAudio">
<!-- 			<audio class="video" :src="audioUrl"
				@error="videoErrorCallback"   name="current.name" author="current.author" ></audio> -->
		...
		<view class="top" v-if="!changeView">
			...
			<view class="course-start" v-if="pageData?.courseType==2&&isCourseStart(pageData?.startTime)">
				<!-- <text class="course-activity-tag" :style="{color:themeFontColor}">限时活动</text> -->
				<text class="course-activity-time">
					{{`据授课`}}
					<text class="time-style">{{day?day:''}}</text>天
					<text class="time-style">{{hours?hours:''}}</text>:
					<text class="time-style">{{Minutes?Minutes:''}}</text>:
					<text class="time-style">{{Seconds?Seconds:''}}</text>
				</text>
			</view>
			<view class="top-box">
				<label class="face-course-box-tag" v-if="pageData?.courseType==1">线上课</label>
				<label class="face-course-box-tag" v-else>直播课</label>
				<text class="top-box-title">{{pageData?.name}}</text>
				<view class="top-box-count">
					<text>{{`${pageData?.threeCateName?pageData?.threeCateName:''} `}}</text>
					<text>{{}}</text>
					<view>{{`| 授课时间：${pageData?.startTime?pageData?.startTime?.substr(5,2):''}月${pageData?.startTime?pageData?.startTime?.substr(8,2):''}日 ${pageData?.startTime?pageData?.startTime?.substr(12,5):''}-${pageData?.startTime?pageData?.endTime?.substr(12,5):''}`}}</view>
				</view>
				<view class="course-service">
					<text class="current_price" v-if="pageData?.costType==1" style="color: #67C23A;">{{'免费'}}</text>
					<text class="current_price" v-else-if="pageData?.costType==2" style="color: #67C23A;">{{'限时免费'}}</text>
					<text class="current_price" v-else-if="pageData?.costType==3" style="color: #395AE1;">{{'会员'}}</text>
					<text class="current_price" v-else-if="pageData?.costType==4" style="color: #EE5142;" v-show="(payRule.aliPayStatus==1||payRule.wechatPayStatus==1||payRule.iosPayStatus==1||payRule.learnPayStatus==1)&&payRule.isClosePay==0">
						...
						<text class="original_cost" v-if="pageData?.isActivity==1">{{'￥'+pageData?.originalPrice}}</text>
					</text>
				</view>
			</view>
		</view>
							...
							<view class="exercise" v-if="j?.isExercise==1">
								<view class="exercise-left">
									...
									<view class="exercise-itme">
										<view>{{j?.name}}</view>
										<text>{{`已做${j?.useQuestionCount?j?.useQuestionCount:''}题 | 错误${j?.errorQuestionCount?j?.useQuestionCount:''}题`}}</text>
									</view>
								</view>
		...
		<view class="courseware" v-show="currentTabs==2">
			...
			<template v-for="(itme,index) in pageData?.coursewareVOList">
				<view class="courseware-itme">
					...
					<view class="courseware-itme-name" @click="onDownload(itme.fileUrl)">{{itme.fileName}}</view>
					<!-- #ifdef  H5 -->
				...
				<view class="course-exam-itme">
					...
					<view class="course-exam-message">
						...
						<view class="course-exam-option">
							<text class="course-exam-count">{{`${itme.examTime?itme.examTime:''}分钟 | ${itme.examCount?itme.examTime:''}道题目`}}</text>
			...
			<view class="bottom-box" v-if="isShowbutton">
				<view style="display: flex;align-items: center;">
					...
					<text class="current_price" >
						{{`预约人数:${pageData?.studyCount?pageData?.studyCount:''}`}}
					</text>
				</view>
...
<script setup>
	import {onLoad,onShow,onPullDownRefresh,onUnload,onReachBottom,onReady,onHide} from "@dcloudio/uni-app";
	import {ref,getCurrentInstance,nextTick,onBeforeUnmount,onUnmounted,watch } from "vue"
	import {getOnlineCourseDetails,listForCourseCommentPage,getLiveCourseDetails,addCollectionCourse,getCourseSectionDetails,addCourseUserStudyRecord} from "../Course.api.ts"
	...
	async function setDetailsById(){
		if(courseType==1){
			await getOnlineCourseDetails({courseId:id}).then(res=>{
				pageData.value = res.result
				remainingTime.value = res.result?.remainingTime
			})
		}else if(courseType==2){
			await getLiveCourseDetails({courseId:id}).then(res=>{
				pageData.value = res.result
				remainingTime.value = res.result?.remainingTime
			})
		}

		await listForCourseCommentPage({courseId:id,commentType:1}).then(res=>{
			
			scoreList.value = res.result.records
		})
		await listForCourseCommentPage({courseId:id,commentType:2}).then(res=>{
	...
	function showtime(){
	    let nowtime = new Date(),  //获取当前时间
	        ...
	        // endtime = new Date(remainingTime.value?.replace(/-/g, "/"));  //定义结束时间
	        endtime = new Date(pageData.value.startTime?.replace(/-/g, "/"));  //定义结束时间
	...
	async function onWatch(e,i,j){
		...
		if(e.sectionType == 5){
			await getCourseSectionDetails({sectionId:e.id}).then(res=>{
				...
				console.log(audioUrl.value.videoUrl,'videoUrl.value');
			audioUrl.value.videoUrl = 'https://videocdn.taobao.com/oss/ali-video/d6bc4ae3eb3c866bee9903d47d1210c6/video.mp4'
			})
	...
	function isCourseStart(time){
		if(time){
			let nowTime = new Date().getTime()
			let oldTime = new Date(time.replace(/-/g, "/")).getTime()
			return nowTime>oldTime?false:true
		}
	}
	...
	function onCollection(e){
		...
		addCollectionCourse(param).then(res=>{
			if(res.code ==200 ){
	...
	function goContact(){
		...
		if(current.value==0){
			uni.navigateTo({
				url:'/course/contact/CourseScore'+'?id='+id
			})
		}else if(current.value==1){
			uni.navigateTo({
				url:'/course/contact/CourseAnswer'+'?id='+id+'&courseName='+pageData.value.name
			})
		}
	}
	...
	function goOrder(){
		let category = pageData.value.courseType=='1'?'1':'3'
		uni.navigateTo({
			url:'/personal/order/OrderDetails?isOrder=0'+'&id='+pageData.value?.id+'&buyMonth='+pageData.value.coursePriceVOList[selectCurrentBoxItem.value]?.specsCount+'&category='+category
		})
	}
</script>
