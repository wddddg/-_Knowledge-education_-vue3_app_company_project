	...
	<view class="container" :style="{'--theme-color':themeColor,'--theme-font-color':themeFontColor}">
		...
		<view class="user-info">
			...
			<view class="pd34">
				...
				<view>
					<u-input v-model="form.password" type="password" placeholder="请输入密码" input-align="right"
						:password-icon="true"></u-input>
				</view>
			</view>
		</view>
		<view class="user-info-btn-box">
			<view class="user-info-btn" :style="{background: verifyTure && passwordVerifyTure ?buttonColor:'#C0C7D1'}"
				@click="changePhone">
				提交
			</view>
		</view>
...
<script setup>
	...
	const passwordVerifyTure = ref(false)
	const oldPhone = ref('')
	...
	const changePhone = async () => {
		...
		if (!passwordVerifyTure.value && !verifyTure.value) return
		locking.value = false
		if (sendCodeValue.value === form.value.verify) {
			await updatePwd(params).then(res => {
				if(res.code === 200){
					uni.showToast({
						title:'修改成功'
					})
