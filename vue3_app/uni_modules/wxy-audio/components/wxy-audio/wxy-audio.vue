	...
	<view class="audio-warp" :style="[style]">
		...
		<view class="audio-con">
			<view class="audio-time-info">
				...
				<slider class="audio-slider" :block-size="12" block-color="#00e3ab" activeColor="#00E3AB" :value="value" @changing="_seeking" @change="_seeked" />
...
export default {
	...
	watch: {
		async play(n) {
			if (n) await _this.innerAudioContext.play();
			else await _this.innerAudioContext.pause();
		},
		src() {
	...
	methods: {
		...
		contextInit() {
			...
			setTimeout(()=>{
			  console.log(innerAudioContext.duration,'795102')//微信要有src才能调用
			},2000)
			//设置报错，提示它只是只读属性
			...
			innerAudioContext.onCanplay(() => {
				...
				setTimeout(()=>{
					if (innerAudioContext.duration !== 0){
						...
						console.log(innerAudioContext.duration,'792')//2.795102
		...
		_seeked(e) {
			...
			_this.innerAudioContext.seek((e.detail.value / 100) * this.duration);
		},
		getDuration() {
			//返回总时间
